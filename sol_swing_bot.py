...
DIP_MIN        = 0.02          # buy SOL after 2%+ pullback from recent high
...
def get_buy_size(price: float, dry_run: bool) -> float:
    """How much SOL to BUY with available USD using dynamic minimums."""
    if dry_run:
...
def get_sell_size(dry_run: bool) -> float:
    """How much SOL to SELL from existing holdings using dynamic minimums."""
    if dry_run:
        # For dry run, simulate selling minimum amount
        min_info = calculate_order_size(PAIR, 100, available_asset=0.1)  # rough price
        return min_info.get('volume', 0.1)
    
    balances = get_balance()
    available_sol = float(balances.get(ASSET, 0)) - RESERVE_SOL
    
    if available_sol <= 0:
        return 0.0
    
    price = float(get_ticker(PAIR).get("c", [0])[0])
    order_info = calculate_order_size(PAIR, price, available_asset=available_sol)
...
def run(dry_run: bool = False):
    ...
    if not dry_run:
        scan_and_protect(
            {PAIR: {"asset": ASSET, "reserve": RESERVE_SOL}},
            PROFIT_PCT, STOP_PCT
        )
